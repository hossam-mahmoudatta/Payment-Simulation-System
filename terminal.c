...
EN_terminalError_t getTransactionDate(ST_terminalData_t* termData) {
	printf("Retrieving System's Current Date...\n");
...
EN_terminalError_t isCardExpired(ST_cardData_t cardData, ST_terminalData_t termData) {
	printf("Checking Your Card Expiry Date Validity...\n");
	...
	char* currentMonth[2] = { 0 };
	char* currentYear[2] = { 0 };
	...
	if (expYear_Integer < currentYear_Integer) {
		...
		printf("	Transaction will not proceed, please try again with a valid card.\n");
		exit(EXPIRED_CARD);
	}
	else if (expYear_Integer == currentYear_Integer) {
		if (expMonth_Integer < 13 && expMonth_Integer > 0) {
			if (expMonth_Integer < currentMonth_Integer) {
				...
				printf("	Your Card is Expired, Transaction Will not Succeedd.\n");
				exit(EXPIRED_CARD);
			}
			else {
			...
			printf("	Card's Month is Invalid!\n");
			//printf("*** Transaction can now proceed!\n");
			exit(WRONG_DATE);
		}
	}
...
EN_terminalError_t getTransactionAmount(ST_terminalData_t* termData) {
	...
	while (True) {
		...
		if (termData->transAmount <= 0) {
			...
			Counter++;
			if (Counter > 2) {
				printf("You entered an INVALID amount several times!\n");
				printf("Program will now be terminated.\n\n");
				exit(INVALID_AMOUNT);
			}
			else if (Counter <= 2) {
				printf("Transaction amount is invalid. Try again.\n");
				continue;
			}
		}
		else {
...
EN_terminalError_t setMaxAmount(ST_terminalData_t* termData) {
	...
	int Counter = 0;
	while (True) {
		printf("Enter the Maximum Allowed Transaction Amount Per Day: ");
		...
		if (termData->maxTransAmount <= 0) {
			...
			Counter++;
			if (Counter > 2) {
				printf("You entered an INVALID Daily Limit several times!\n");
				printf("	Program will now be terminated.\n\n");
				exit(INVALID_MAX_AMOUNT);
			}
			else if (Counter <= 2) {
				printf("Daily Limit amount is invalid. Try again.\n");
				continue;
			}
		}
		else {
			printf("	Maximum Allowed Transaction is %f\n", termData->maxTransAmount);
			return TERM_OK;
		}
...
EN_terminalError_t isBelowMaxAmount(ST_terminalData_t* termData) {
	printf("Checking if Amount Entered is Valid...\n");
	if (termData->maxTransAmount < termData->transAmount) {
		printf("	Your Transaction Exceeded the Maximum Daily Amount Allowed.\n");
		printf("Program will now be terminated.\n\n");
		exit(EXCEED_MAX_AMOUNT);
	}
	else {
		printf("	Transaction Amount is Within the Daily Limit!\n");
		printf("		Please Wait While We Work on Your Request..\n");
		return TERM_OK;
	}
}
